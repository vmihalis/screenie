...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 2: Configure npm trusted publisher</name>
  <action>
...
3. Configure trusted publisher with EXACT values (case-sensitive):
   - **Repository owner:** memehalis
   - **Repository name:** responsiveness-mcp (or your actual repo name - check GitHub URL)
   - **Workflow filename:** publish.yml (include .yml extension)
   - **Environment name:** Leave empty (not using GitHub environments)

CRITICAL: The workflow filename must EXACTLY match what you created in Task 1. Case-sensitive, include .yml extension.
...
<task type="auto">
  <name>Task 3: Create GitHub release to trigger publish</name>
  <files></files>
  <action>
...
- [Landing Page](https://landing-gilt-psi-18.vercel.app)"
```
...
1. Trusted publisher config on npm matches exactly (repo owner, repo name, workflow filename)
...
<task type="auto">
  <name>Task 4: Verify npm publication and provenance</name>
  <files></files>
  <action>
